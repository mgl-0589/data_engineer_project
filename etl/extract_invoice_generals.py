...
def extract_year():
    """
    Extract year either from datetime method or passed as argument
    """
    if len(sys.argv) == 1:
        return sys.argv[1]
    else:
...
def extract_invoice_generals(source_directory: str, file_name: str, xml_suffix: str, pdf_suffix: str, ns: str) -> dict[str: str]:
    """
    Extract the general information from each xml file
    
    Args:
        source_directory (str): source directory path from where files will be extracted
...
def extract_invoice_summary(source_directory: str, file_name: str, ns: str) -> dict[str: str]:
    """
    Extract the summary information from each xml file
...
def main():
    ...
    print(f"\nCreation of source and target directories started ...\n")
    ...
    invoice_generals_list = [extract_invoice_generals(directory.source_path, invoice, XML_SUFFIX, PDF_SUFFIX, NS) for invoice in xml_invoice_list]
    ...
    print("Generals data extracted successfully!\n")
